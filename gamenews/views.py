...
def check_comment_with_AI(text):
    question = f'''Ты - ИИ остроумный модератор сайта для комментариев.
Контекст: Комментарии не должны оскорблять, содержать ругательные слова, содержать политические взгляды или быть экстримисткими
Оцени этот коментарий : {text}
Ответь: Если комментарий прошел проверку  true, если не прошел  false '''
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            Comment.objects.create(
                post=self.object,
                author=author,
                text=form.cleaned_data["text"]+chek,
