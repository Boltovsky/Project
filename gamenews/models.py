...
def new_comment_create(sender, instance:Comment, created, **kwargs):
    if created and instance.verify is False:
        send_message(f'''Получен новый коментарий!
<b>ОТ</b>:{instance.author.username}
<b>ПОСТ</b>:{instance.post.title}
<b>ТЕКСТ</b>:<code>{instance.text}</code>
<b>ДАТА</b>:{instance.date}
''', instance.id)
        
        # print(f'Новый комент с текстом: {instance.text}')
